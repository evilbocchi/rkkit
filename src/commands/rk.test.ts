...
describe("rk command", () => {
    ...
    describe("rkCommandHandler and rkCommandHandlerSync", () => {
        ...
        it("should automatically initialize rokit.toml if autoInit is true", async () => {
            ...
            vi.mocked(readFile).mockResolvedValue(
                'tools = { lune = "rojo-rbx/lune@0.0.0" }',
            );
            vi.mocked(parseToml).mockReturnValue({
                tools: { lune: "rojo-rbx/lune@0.0.0" },
            });
            ...
            expect(logger.info).toHaveBeenCalledWith(
                expect.stringContaining("automatically initializing one"),
            );
            expect(mockRokitCommandHandler).toHaveBeenCalledWith(
                expect.objectContaining({ args: ["init"] }),
            );
        });
        ...
        it("should handle semver satisfaction", async () => {
            ...
            await rkModule.rkCommandHandler({ tool: "lune" });
            expect(spawn).toHaveBeenCalledWith(
                expect.stringContaining("0.21.1"),
                [],
                expect.any(Object),
            );
        });
        ...
        it("should exit if no satisfying version is found after install", async () => {
            ...
            mockRokitCommandHandler.mockResolvedValue({ status: 0 });
            await expect(
                rkModule.rkCommandHandler({ tool: "lune" }),
            ).rejects.toThrow("process.exit(1)");
            expect(logger.error).toHaveBeenCalledWith(
                expect.stringContaining("could not find an installed version"),
            );
        });
        ...
        it("should exit if rokit install fails", async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith(
                expect.stringContaining("failed to install tools via rokit."),
            );
        });
        ...
        it("should handle missing tool storage directory", async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith(
                expect.stringContaining("tool storage directory not found"),
            );
        });
        ...
        it("should scan directory for an executable if default names are not found", async () => {
            ...
            await rkModule.rkCommandHandler({ tool: "lune" });
            expect(spawn).toHaveBeenCalledWith(
                expect.stringContaining("random-exe"),
                expect.any(Array),
                expect.any(Object),
            );
        });
